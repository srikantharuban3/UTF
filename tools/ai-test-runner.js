...
/**
 * AI Test Runner - Executes tests based on Testsuite.md using Playwright
 */
class AITestRunner {
  constructor() {
    this.browser = null;
...
<head>
    ...
    <style>
        ...
        .header { background: ${this.testResults.status === 'PASSED' ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' : 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)'}; color: white; padding: 30px; text-align: center; }
        .content { padding: 30px; }
        .status-badge { display: inline-block; padding: 8px 16px; border-radius: 20px; font-weight: bold; color: white; background: ${this.testResults.status === 'PASSED' ? '#10b981' : '#ef4444'}; }
        ...
        .step { display: flex; align-items: center; padding: 15px; margin: 10px 0; border-radius: 8px; background: #f9fafb; border-left: 4px solid ${this.testResults.status === 'PASSED' ? '#10b981' : '#ef4444'}; }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Automated Testing with Playwright</p>
